...
- Existing file operations have moved into `Bash.Filesystem.LocalDisk` and there's now a behaviour to implement if more backends are needed.
...
- Happy MAR10 day
- `Bash.run` would take options, but not pass them through. Now it does.

## 0.3.3 (2026-03-09)

- Fixing compile warnings and `mix format` as a plugin issues.
