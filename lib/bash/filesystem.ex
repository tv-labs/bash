defmodule Bash.Filesystem do
  ...
  def from_state(%{filesystem: fs}), do: fs
  def from_state(_), do: {Bash.Filesystem.LocalDisk, nil}

  @spec exists?(t(), String.t()) :: boolean()
  def exists?({mod, config}, path), do: mod.exists?(config, path)

  @spec dir?(t(), String.t()) :: boolean()
  def dir?({mod, config}, path), do: mod.dir?(config, path)

  @spec regular?(t(), String.t()) :: boolean()
  def regular?({mod, config}, path), do: mod.regular?(config, path)

  @spec stat(t(), String.t()) :: {:ok, File.Stat.t()} | {:error, term()}
  def stat({mod, config}, path), do: mod.stat(config, path)

  @spec lstat(t(), String.t()) :: {:ok, File.Stat.t()} | {:error, term()}
  ...
  @spec wildcard(t(), String.t(), keyword()) :: [String.t()]
  def wildcard({mod, config}, pattern, opts), do: mod.wildcard(config, pattern, opts)

  @spec read_link(t(), String.t()) :: {:ok, String.t()} | {:error, term()}
  ...
  @spec read_link_all(t(), String.t()) :: {:ok, String.t()} | {:error, term()}
