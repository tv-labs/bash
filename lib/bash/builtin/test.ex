defmodule Bash.Builtin.Test do
  ...
  def file_exists?(path, state) do
    ...
    Bash.Filesystem.exists?(state.filesystem, full_path)
  end

  @doc false
  def file_regular?(path, state) do
    full_path = resolve_path(path, state.working_dir)
    Bash.Filesystem.regular?(state.filesystem, full_path)
  end

  @doc false
  def file_directory?(path, state) do
    full_path = resolve_path(path, state.working_dir)
    Bash.Filesystem.dir?(state.filesystem, full_path)
  end
  ...
  def file_symlink?(path, state) do
    ...
    case Bash.Filesystem.lstat(state.filesystem, full_path) do
  ...
  def file_readable?(path, state) do
    ...
    case Bash.Filesystem.stat(state.filesystem, full_path) do
  ...
  def file_newer_than?(file1, file2, state) do
    ...
    with {:ok, stat1} <- Bash.Filesystem.stat(state.filesystem, path1),
         {:ok, stat2} <- Bash.Filesystem.stat(state.filesystem, path2) do
