defmodule Bash.VirtualFilesystemTest do
  ...
  defp start_vfs_session(context, initial_files, opts \\ []) do
    ...
    session_opts =
      if opts[:restricted],
        do: Keyword.put(session_opts, :options, %{restricted: true}),
        else: session_opts
  ...
  describe "fully sandboxed session" do
    test "restricted mode + VFS blocks commands and uses virtual files", context do
      {session, _fs} =
        start_vfs_session(
          context,
