defmodule Bash.BackgroundTest do
  ...
  describe "jobs builtin" do
    ...
    test "jobs -l shows PIDs", %{session: session} do
      ...
      # Wait a moment for job to get OS PID
      Process.sleep(50)
    ...
    test "jobs returns empty when no jobs", %{session: session} do
      result = run_script(session, "jobs")
  ...
  describe "wait builtin" do
    test "wait blocks until job completes", %{session: session} do
      run_script(session, "sleep 0.1 &")
    ...
    test "wait returns exit code of last job", %{session: session} do
    ...
    test "wait with no jobs succeeds immediately", %{session: session} do
      result = run_script(session, "wait")
  ...
  describe "fg builtin" do
    test "fg with no jobs returns error", %{session: session} do
    ...
    test "fg brings background job to foreground", %{session: session} do
      run_script(session, "sleep 0.1 &")
      ...
      # Should have waited for sleep to complete
      assert elapsed >= 50
      assert result.exit_code == 0
    end
  end
